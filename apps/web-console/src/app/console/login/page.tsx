  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      <div className="w-full max-w-md p-8 bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/20">
        <h1 className="text-3xl font-bold text-white text-center mb-2">歡迎回來</h1>
        <p className="text-gray-300 text-center mb-8">道之自然·命理AI系統</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {mode === 'email' ? (
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">手機號碼</label>
              <input
                type="tel"
                value={phone}
        ...
        <div className="mt-4">
          <button
            ...
            disabled={loading}
            className="w-full py-2 text-sm text-gray-400 hover:text-white border border-white/10 rounded-lg hover:border-white/30 transition-all"
          >
            以遊客身份體驗
          </button>
        </div>
        ...
        <p className="mt-6 text-center text-gray-400 text-sm">
          還沒有帳號？
          <a href="/console/register" className="text-purple-400 hover:text-purple-300 ml-1">立即註冊</a>
        </p>
      </div>
    </div>
  );
